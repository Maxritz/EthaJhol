...
`vg-gguf-engine` is a portable GGUF inference engine with a compact C99 systems layer and a complete C++17 model-generation layer. It is self-contained: no external LLM runtime is required. It uses Vulkan when available, and keeps large-model operation explicit through mmap/lazy loading, bounded tensor residency, paged KV state, radix prefix reuse, and plugin interfaces.

The project combines a native GGUF and storage core with a native full graph runtime. The native layer provides metadata inspection, typed tensor geometry validation, disk-backed tensor leases, host LRU caching, prefetch and streaming hooks, paged KV allocation, immutable sequence forks, radix prefix snapshots, tracing, runtime plugins, and a headless Vulkan quantized matvec test. The full layer exposes a stable C streaming API and a CLI that performs real model tokenization, prompt evaluation, decoding, sampling, and token-piece streaming through the engine's own transformer graph evaluator.
...
| CPU inference | Available with `-ngl 0`. |
...
| Dense and hybrid model families | Supported natively. Unsupported files fail with diagnostics rather than silently falling back to incorrect math. |
...
The default build includes the native inference engine and (optionally) the Vulkan backend:

```sh
cmake -S . -B build -DVG_ENABLE_VULKAN=ON -DCMAKE_BUILD_TYPE=Release
cmake --build build --config Release
```

For a minimal embeddable systems-only build, disable the full graph runtime:
...
./build/vg-run \
  ...
  -n 128 -c 8192 --temp 0.8 --top-k 40 --top-p 0.95
```

Use `--no-mmap` to disable mapping, `--threads N` to set CPU generation threads, `--seed N` for deterministic output, and `-c N` to select the context window. The C API in `include/vg/full_engine.h` lets a host application receive each token piece through `VG_TokenCallback`.

On Windows 11, build with Visual Studio or clang-cl through the same CMake project and launch `vg-run.exe` directly.

## Native core tools

`vg-engine inspect model.gguf` prints GGUF architecture, tensor geometry, and storage information. `vg-engine stream model.gguf TENSOR` exercises a bounded lazy tensor lease. `vg-engine vk-selftest` validates the native Vulkan dispatch. `vg-engine --cpu-infer model.gguf` runs native CPU inference. `vg-tests` covers paged KV allocation, sequence fork behavior, and radix lookup. `vg-quant-tests` covers scalar conversion and legacy packed quantization. `vg-bench` measures page growth and radix lookup behavior.
